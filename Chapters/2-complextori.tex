...
\begin{example}
Let $(X,\mathcal{O}_X)$ be a ringed space, and $\mathcal{F}$ a $\mathcal{O}_X$-module. Let $\Gamma(X,-)$ be the ``global sections'' functor, $\mathcal{F}\mapsto \mathcal{F}(X)$, from $\mathbb{O}_X$-modules to $R$-modules for $\mathcal{O}_X(X)$. Here there is the Grothedieck derived cohomology functor, but there is also a more concrete one, the Cech cohomology. These are not equal in general! Fortunately they are equal in the settings we will be interested in.
\end{example}
