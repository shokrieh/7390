...
\subsection{Next time} We will talk about the exponential exact sequence, and generalize to $X$. We will then get a similar long exact sequence, but here $H^2(X,\ZZ)$ is nontrivial, and the map $H^1(X,\mathcal{O}_X^\times)\to H^2(X,\ZZ)$ is extremely important! Our next goal is the Appell-Humbert theorem, which is actually due to Lefschetz.