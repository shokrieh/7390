...
$$X_L(\lambda,v)=f(\lambda) \cdot e^{2\pi i \ell(v)-2\pi i \ell(v+\lambda)}.$$
Next time we will check that $\lambda_L\in \Hom(\Lambda,T_1)$ is independent of $v$ and has values in $T_1$ and therefore $f$ is in $Z^1(M,\Lambda)$ and so $\alpha'$ is a surjection. 
