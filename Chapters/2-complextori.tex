...
\section{Lecture: Feb 18, 2016}
Last time: If $H^n(BG,\underline{M})\isom H^n(G,M)$, then $M$ has a triial $G$ action.
Also $H^1(X,\mathcal{O}_X^\times)$ means invertible elements with respect to multiplication. (For example, if $f$ is analytic with $f(0)\not=0$, then $1/f$ is analytic when close to zero.)

Goal of today: prove the Theorem from last time.
...
$$h(\mu)\cdot \lambda \times h(\lambda \mu)^{-1}\times h(\lambda)=1.$$
...
$$f(\lambda,\tilde{x})=\frac{h_1(\lambda,\tilde{x})}{h_1(\tilde{x})}$$
and we define $H^1(G,M)=Z^1/B^1$ like before.

Now, each $f\in Z^1(G,M)$ gives a Line Bundle on $X$. Let $L=\tilde{X}\times \CC\to \tilde{X}$ be the line bundle map. Then $G=\pi_1(X)$ acts on $\tilde{X}$, and so acts on $L$: $\lambda\cdot(\tilde{x},t)=(\lambda\cdot \tilde{x},f(\lambda,\tilde{x})t)$.
...
There is a theorem saying that complex manifolds came from complex manifolds moding out free proper disc actions. Therefore, the induced $L'\to X$ is a holomorphic line bundle. And this gives a map $Z^1(G,M)\to H^1(X,\mathcal{O}_X^\times)$. 
...
$\pi_j^{-1}(x)=\lambda_{ij}\cdot \pi_i^{-1}(x)$. It is clear that $\lambda_{ij}\cdot \lambda_{jk}=\lambda_{ik}$. Therefore, now we have a map $Z^1(G,M)\to Z^1(X,\mathcal{0}_X^\times)$ (note that $Z^1(X,\mathcal{O}_X^\times)$ surjects onto $H^1(X,\mathcal{O}_X^\times)$) given by 
...
It is easy to check that $\ker(\phi_1\supset B^1(G,M))$. [farbod fill in detail] 
...
We know that $\pi^*L$ is trivial. So we fix $\alpha:\pi^*L\isomto \tilde{X}\times \CC$. (Note that $G$ acts on $\pi^*L$, and hence via $\alpha$, $G$ also acts on $\tilde{X}\times G$.) For all $\lambda\in G$, there exists automorphisms $\phi_\lambda$ of $\tilde{X}\times \CC$ such that 
...
Theorem: Let $\mathcal{F}$ be a sheaf of abelian groups on $X$. Let $G=\pi_1(X)$ and $M=H^0(\tilde{X},\pi^*\mathcal{F})$. Then for each $n\geq 0$, there exist canonical homomorphisms $\phi_n:H^n(G,M)\to H^n(X,\mathcal{F})$. Moreover, 
\begin{itemize}
...
Our next goal: describe global sections $H^0(X,L)=\Gamma(L,X)$ in terms of $\phi_1(f)=L$, where $L\in \ker(H^1(X,\mathcal{O}_X^\times)\to H^1(\tilde{X},\mathcal{O}_X^\times))$. 

Since $\phi^*L$ is trivial, if we fix $\alpha:\phi^*L\isomto \tilde{X}\times \CC$, we get $f\in Z^1(G,M)$. Observe that there exists a canonical isomorphism between $H^0(X,L)\isomto H^0(\tilde{X},\pi^*L)^G$. But now, via $\alpha$, we have $H^0(\tilde{X}m\pi^*L)^G\isom H^0(\tilde{X},\tilde{X}\times \CC)^G$, which is explicitly given by
$$\phi_\lambda(\tilde{x},t)=(\lambda\cdot \tilde{x}, f(\lambda,\tilde{x})t).$$

And $H^0(\tilde{X},\tilde{X}\times \CC)^G=\{\vartheta:\tilde{X}\overset{hol}{\to}\CC: \vartheta(\lambda,\tilde{x})=f(\tilde{\lambda},\tilde{x})\vartheta(x)\}$ and $f$ here is called a ``factor of automorphy''. 

Important remark: all the above depends on $\alpha$. 

Remark: One can check functoriality. 


Back to $X=V/\Lambda$ complex torus. 

Proposition: Every holomorphic line bundle on $X$ pulls back to a trivial bundle on $V$. 
Sketch of proof: 
\begin{itemize}
...
\item $\overline{\partial}$-Poincare lemma
\item $H^2(V,\ZZ)=0$.
\end{itemize}

This induces the exact sequence:
$$\cdots \to H^1(V,\mathcal{O}_V)\to H^1(V,\mathcal{O}_V^\times)\to H^2(V,\ZZ)\to \cdots$$
First term is trivial by the $\overline{\partial}$-Poincare lemma, and last term is trivial by $H^2(V,\ZZ)=0$. Therefore, the middle is also trivial. 

Corollary:
$\Pic(X):=H^1(X,\mathcal{O}_X^\times)\isom H^1(\Omega,H^0(V,\mathcal{O}_V^\times))$.

Griffiths-Harris? [fix]

Next time: next goal: Appell-Humbert (Lefschetz)
